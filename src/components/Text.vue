<template>
    <section id="textType" :class="['text__wrap section', element]">
        <span>텍스트 유형01</span>
        <h2>{{ title }}</h2>
        <div class="text__inner container">
            <div v-for="(text, index) in textInfo" :key="key" :class="'text t' + index">
                <h3 class="text__title">{{ text.tilte }}</h3>
...
export default {
    props: {
        element: String, // Element 속성 정의
        title: String, // 문자열 타입의 title 속성 정의
    },
    data: function () {
        return {
            textInfo: [
...
.text__title::before {
    ...
    width: 60px;
    height: 60px;
    background-image: url(../assets/img/text_icon.svg);
}
